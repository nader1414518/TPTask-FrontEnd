<template>
    <div class="page-container">
        ...
        <div>
            ...
            <!-- Submit button -->
            <button type="button" class="btn btn-primary btn-block mb-4" @click="editRecord">Edit</button>
        </div>
    </div>
</template>
        ...
        methods: {
            ...
            async getRecordData() {
                ...
                try {
                    ...
                    var recordRes = await this.axios.get(utils.apiUrl + '/records/' + this.$route.params.id, config);
                    
                    if (recordRes.data !== undefined && recordRes.data !== null)
                    {
                        this.Name = recordRes.data.name;
                        this.Email = recordRes.data.email;
                        this.Phone = recordRes.data.phone;
                        this.ImgUrl = recordRes.data.imgUrl;
                    }
                } catch (error) {
            ...
            async editRecord()
            {
                ...
                try {
                    ...
                    await this.axios.put(utils.apiUrl + '/records/' + this.$route.params.id, {
                    ...
                    if (error.response !== undefined && error.response !== null)
                    {
                        if (error.response.data !== undefined && error.response.data !== null)
                        {
                            ...
                            {
                                if (error.response.data.errors['id'] !== undefined && error.response.data.errors['id'])
                                {
                                    toaster.error(error.response.data.errors['id']);
                                }
                            }
                        }
                    }
